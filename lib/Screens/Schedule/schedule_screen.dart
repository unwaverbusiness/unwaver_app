...
class _ScheduleScreenState extends State<ScheduleScreen>
  ...
  String _routineContext =
      'date'; // 'date', 'standard', 'weekday', 'weekend', 'monday', etc.
  ...
  void _onItemDropped(ScheduleItem item, TimeOfDay dropTime) {
    setState(() {
      ...
      for (var list in _routineTemplates.values) {
        list.removeWhere((e) => e.id == item.id);
      }
      ...
      if (_routineContext == 'date') {
        final normalizedDate =
            DateTime(_selectedDay.year, _selectedDay.month, _selectedDay.day);
        final updatedItem =
            item.copyWith(scheduledDate: normalizedDate, startTime: dropTime);
        ...
        final updatedItem = ScheduleItem(
            ...
            startTime: dropTime,
            scheduledDate: null);
        if (_routineTemplates[_routineContext] == null) {
          _routineTemplates[_routineContext] = [];
        }
        _routineTemplates[_routineContext]!.add(updatedItem);
        _routineTemplates[_routineContext]!.sort((a, b) =>
            (a.startTime!.hour * 60 + a.startTime!.minute)
                .compareTo(b.startTime!.hour * 60 + b.startTime!.minute));
      }
    });
  ...
  void _removeToPool(ScheduleItem item) {
    setState(() {
      ...
      _unscheduledItems
          .add(item.copyWith(scheduledDate: null, startTime: null));
    });
  }
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('"${newItem.title}" added to Routine Pool')));
              },
                ...
                child: DropdownButton<String>(
                  ...
                  items: [
                    DropdownMenuItem(
                        value: 'date',
                        child: Text(
                            "Specific Date: ${DateFormat('MMM d, yyyy').format(_selectedDay)}")),
                    const DropdownMenuItem(
                        value: 'standard',
                        child: Text("Template: Standard Routine")),
                    const DropdownMenuItem(
                        value: 'weekday',
                        child: Text("Template: Weekdays (Mon-Fri)")),
                    const DropdownMenuItem(
                        value: 'weekend', child: Text("Template: Weekends")),
                    const DropdownMenuItem(
                        value: 'monday', child: Text("Template: Every Monday")),
                    const DropdownMenuItem(
                        value: 'tuesday',
                        child: Text("Template: Every Tuesday")),
                    const DropdownMenuItem(
                        value: 'wednesday',
                        child: Text("Template: Every Wednesday")),
                    const DropdownMenuItem(
                        value: 'thursday',
                        child: Text("Template: Every Thursday")),
                    const DropdownMenuItem(
                        value: 'friday', child: Text("Template: Every Friday")),
                  ],
                  onChanged: (val) {
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return DragTarget<ScheduleItem>(
                ...
                onAcceptWithDetails: (details) => _onItemDropped(
                    details.data, TimeOfDay(hour: hour, minute: 0)),
            ...
            children: [
              Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text("ROUTINE POOL",
                        style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.bold,
                            letterSpacing: 1.2)),
                    Text("Drag to assign • Tap to edit",
                        style:
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Draggable<ScheduleItem>(
                      ...
                      childWhenDragging:
                          Opacity(opacity: 0.3, child: _buildPoolCard(item)),
                      child: GestureDetector(
                          onTap: () => _editItem(item),
                          onDoubleTap: () => _deleteItemPermanently(item),
            ...
            Draggable<ScheduleItem>(
              ...
              feedback: Material(
                  ...
                  child: SizedBox(
                      ...
                      child: _buildScheduledBlock(item, isDragging: true))),
              childWhenDragging: Opacity(
                  opacity: 0.0,
                  child: Icon(Icons.drag_indicator,
                      color: Colors.grey.shade400, size: 24)),
              child: Icon(Icons.drag_indicator,
                  color: Colors.grey.shade500, size: 24), // Instant grab area!
            )
          else
            Icon(Icons.drag_indicator,
                color: Colors.transparent, size: 24), // Keep spacing consistent
  ...
  Widget _buildPoolCard(ScheduleItem item, {bool isDragging = false}) {
    return Container(
      width: 140,
      margin: EdgeInsets.symmetric(horizontal: 4, vertical: isDragging ? 0 : 4),
      ...
      child: Column(
        ...
        children: [
          ...
          Text(item.title,
              ...
              overflow: TextOverflow.ellipsis),
          const SizedBox(height: 4),
          Text("${item.type.name} • ${item.durationMinutes}m",
          ...
          child: Row(
            ...
            children: [
              IconButton(
                  icon: const Icon(Icons.chevron_left),
                  onPressed: () =>
                      setState(() => _focusedDay = DateTime(currentYear - 1))),
              Text("$currentYear",
                  style: const TextStyle(
                ...
                child: Container(
                  decoration: BoxDecoration(
                      ...
                      border: isCurrentMonth
                          ? Border.all(color: _goldColor)
                          : Border.all(color: Colors.grey.shade200)),
                  child: Center(
                    child: Text(DateFormat('MMM').format(monthDate),
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                            color: isCurrentMonth ? _goldColor : Colors.black)),
                  ),
                ),
              );
            },
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
              "Connect external calendars to Unwaver to populate your Routine Builder automatically.",
  ...
  Widget _buildIntegrationCard(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
              color: isConnected ? _goldColor : Colors.grey.shade200,
