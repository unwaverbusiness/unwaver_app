...
class _PurposeGeneratorScreenState extends State<PurposeGeneratorScreen> {
  ...
  // Mock User Context
  final String _userGoals =
      "1. Run a marathon. 2. Build a million-dollar business. 3. Read 20 books this year.";
  final String _userHabits =
      "Morning meditation, Coding for 2 hours daily, No sugar diet.";
  ...
  List<Map<String, dynamic>> _parseItemList(dynamic data) {
    ...
    if (data is List) {
      return data.map((e) {
        if (e is String) {
          return {'title': e, 'notes': ''}; // Migrates old strings
        }
        ...
        setState(() {
          ...
          _innerCircle = _parseItemList(data['innerCircle']).map((e) {
            if (!e.containsKey('category')) {
              e['category'] = 'Friends'; // Default fallback
            }
            return e;
          }).toList();
          ...
          if (data['cardVisibility'] != null) {
            final visibility =
                Map<String, dynamic>.from(data['cardVisibility']);
            _cardVisibility =
                visibility.map((key, value) => MapEntry(key, value as bool));

            if (_cardVisibility.containsKey('strengths') ||
                _cardVisibility.containsKey('weaknesses')) {
              _cardVisibility['strengthsAndWeaknesses'] =
                  (_cardVisibility['strengths'] ?? true) ||
                      (_cardVisibility['weaknesses'] ?? true);
          ...
          if (data['cardOrder'] != null) {
            ...
            if (_cardOrder.contains('strengths') ||
                _cardOrder.contains('weaknesses')) {
  ...
  void _showItemDetailDialog({
    ...
    Color selectedColor =
        item['color'] != null ? Color(item['color']) : _goldColor;
    int? selectedIconCode = item['icon'];

    List<String> traits =
        item['traits'] != null ? List<String>.from(item['traits']) : [];
        ...
        child: StatefulBuilder(builder: (ctx, setDialogState) {
          return Container(
            ...
            constraints: BoxConstraints(
                maxWidth: 500, maxHeight: MediaQuery.of(ctx).size.height * 0.9),
            child: Column(
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text("Importance Rank (1-10)",
                            style: TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 14)),
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Slider(
                                ...
                                onChanged: (val) =>
                                    setDialogState(() => rank = val),
                              ),
                            ),
                          ],
                        ),
                        const SizedBox(height: 16),
                        const Text("Visual Customization",
                            style: TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 14)),
                        const SizedBox(height: 8),
                        ...
                        const SizedBox(height: 16),
                        const Text("Traits (Characteristics/Tags)",
                            style: TextStyle(
                                fontWeight: FontWeight.bold, fontSize: 14)),
                        const SizedBox(height: 8),
                            ...
                            IconButton(
                                ...
                                onPressed: () {
                                  if (traitCtrl.text.trim().isNotEmpty) {
                                    setDialogState(() {
                                      traits.add(traitCtrl.text.trim());
                                      traitCtrl.clear();
                                    });
                                  }
                                })
  ...
  void _showAddDialog(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text("Add to $categoryTitle",
  ...
  void _showAddInnerCircleDialog({required String defaultCategory}) {
    ...
    final allCategories =
        _innerCircleSections.map((s) => s['title'] as String).toList();
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (context, setDialogState) {
        return AlertDialog(
          ...
          title: const Text("Add to Inner Circle",
    ...
    switch (key) {
      case 'purpose':
        return _buildFloatingCard(
          ...
          icon: Icons.star,
          goldAccent: true,
          onMagicWand: () => _showBrainstormModal(
              categoryTitle: "Purpose",
              buttonText: "Apply",
              aiContext:
                  "Help the user write a single, powerful sentence that defines their ultimate purpose.",
              onAdd: (val) {
          ...
          child: Focus(
            ...
            child: TextField(
                ...
                decoration: InputDecoration(
                    ...
                    hintText: "What is your ultimate purpose?",
                    hintStyle: TextStyle(color: Colors.grey.shade400))),
          ),
        );
      case 'priorities':
        return _buildFloatingCard(
          ...
          onMagicWand: () => _showBrainstormModal(
              categoryTitle: "Priorities",
              aiContext:
                  "Suggest broad, essential life priorities based on their purpose.",
              onAdd: (val) {
                setState(() => _priorities.add({'title': val, 'notes': ''}));
                _saveFieldToFirebase('priorities', _priorities);
              }),
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Priorities",
      ...
      case 'coreValues':
        return _buildFloatingCard(
          ...
          icon: Icons.diamond,
          onMagicWand: () => _showBrainstormModal(
              categoryTitle: "Core Values",
              aiContext: "Suggest one-word core values.",
              onAdd: (val) {
                setState(() => _coreValues.add({'title': val, 'notes': ''}));
                _saveFieldToFirebase('coreValues', _coreValues);
              }),
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Core Values",
              collectionKey: "coreValues",
              items: _coreValues),
          child: _buildEditableWrap(
              items: _coreValues,
              color: _goldColor,
              categoryTitle: "Core Value",
              collectionKey: "coreValues"),
        );
      case 'identityStatements':
        return _buildFloatingCard(
          ...
          onMagicWand: () => _showBrainstormModal(
              ...
              aiContext:
                  "Suggest powerful statements starting with 'I am', 'I will', or 'I create'.",
              onAdd: (val) {
                setState(
                    () => _identityStatements.add({'title': val, 'notes': ''}));
                _saveFieldToFirebase('identityStatements', _identityStatements);
              }),
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Identity Statements",
              collectionKey: "identityStatements",
              items: _identityStatements),
          child: _buildEditableWrap(
              items: _identityStatements,
              color: Colors.grey.shade800,
              categoryTitle: "Identity Statement",
              collectionKey: "identityStatements"),
        );
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Row(children: [
                            InkWell(
                              onTap: () => _showElementsCustomizeDialog(
                                  categoryTitle: "Strengths",
                            ...
                            InkWell(
                              onTap: () => _showBrainstormModal(
                                  categoryTitle: "Strengths",
                                  aiContext:
                                      "Suggest personal strengths needed for this mission.",
                                  onAdd: (val) {
                                    setState(() => _strengths
                                        .add({'title': val, 'notes': ''}));
                                    _saveFieldToFirebase(
                                        'strengths', _strengths);
                                  }),
                              child: Icon(Icons.auto_awesome,
                                  size: 16, color: _goldColor),
                            ),
                          ])
                        ],
                      ),
                      ...
                      _buildEditableWrap(
                          items: _strengths,
                          color: Colors.green.shade700,
                          categoryTitle: "Strength",
                          collectionKey: "strengths"),
                    ],
                  ),
                ),
                VerticalDivider(
                    color: Colors.grey.shade200, thickness: 1, width: 24),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Row(children: [
                            InkWell(
                              onTap: () => _showElementsCustomizeDialog(
                                  categoryTitle: "Weaknesses",
                                  collectionKey: "weaknesses",
                                  items: _weaknesses),
                              child: Icon(Icons.tune,
                                  size: 16, color: Colors.grey.shade500),
                            ),
                            ...
                            InkWell(
                              onTap: () => _showBrainstormModal(
                                  categoryTitle: "Weaknesses",
                                  aiContext: "Suggest typical blind spots.",
                                  onAdd: (val) {
                                    setState(() => _weaknesses
                                        .add({'title': val, 'notes': ''}));
                                    _saveFieldToFirebase(
      ...
      case 'vices':
        return _buildFloatingCard(
          cardKey: key,
          title: "Vices to Conquer",
          icon: Icons.shield_outlined,
          onMagicWand: () => _showBrainstormModal(
              categoryTitle: "Vices",
              aiContext: "Suggest bad habits that would destroy this mission.",
              onAdd: (val) {
                setState(() => _vices.add({'title': val, 'notes': ''}));
                _saveFieldToFirebase('vices', _vices);
              }),
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Vices", collectionKey: "vices", items: _vices),
          child: _buildEditableWrap(
              items: _vices,
              color: Colors.grey.shade800,
              categoryTitle: "Vice",
      ...
      case 'gratitudes':
        return _buildFloatingCard(
          ...
          onMagicWand: () => _showBrainstormModal(
              categoryTitle: "Gratitude",
              aiContext:
                  "Suggest things to be grateful for to maintain perspective.",
              onAdd: (val) {
                setState(() => _gratitudes.add({'title': val, 'notes': ''}));
                _saveFieldToFirebase('gratitudes', _gratitudes);
              }),
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Gratitudes",
              collectionKey: "gratitudes",
              items: _gratitudes),
          child: _buildEditableWrap(
              items: _gratitudes,
      ...
      case 'innerCircle':
        return _buildFloatingCard(
          ...
          icon: Icons.groups,
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Inner Circle Sections",
              collectionKey: "innerCircleSections",
              items: _innerCircleSections),
      ...
      case 'aspirations':
        return _buildFloatingCard(
          ...
          icon: Icons.star,
          onMagicWand: () => _showBrainstormModal(
              categoryTitle: "Aspirations",
              aiContext: "Suggest future ambitions and aspirations.",
              onAdd: (val) {
                setState(() => _aspirations.add({'title': val, 'notes': ''}));
                _saveFieldToFirebase('aspirations', _aspirations);
              }),
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Aspirations",
              collectionKey: "aspirations",
              items: _aspirations),
          child: _buildEditableWrap(
              items: _aspirations,
              color: Colors.orange.shade700,
              categoryTitle: "Aspiration",
              collectionKey: "aspirations"),
        );
      case 'characteristics':
        return _buildFloatingCard(
          ...
          icon: Icons.person,
          onMagicWand: () => _showBrainstormModal(
              categoryTitle: "Characteristics",
              aiContext: "Suggest defining personal characteristics.",
              onAdd: (val) {
                setState(
                    () => _characteristics.add({'title': val, 'notes': ''}));
                _saveFieldToFirebase('characteristics', _characteristics);
              }),
          onCustomizeElements: () => _showElementsCustomizeDialog(
              categoryTitle: "Characteristics",
              collectionKey: "characteristics",
              items: _characteristics),
          child: _buildEditableWrap(
              items: _characteristics,
              color: Colors.green.shade700,
              categoryTitle: "Characteristic",
              collectionKey: "characteristics"),
        );
      default:
        return null;
                            ...
                            child: ListTile(
                              ...
                              trailing: IconButton(
                                icon: Icon(
                                    ...
                                    color:
                                        isVisible ? _goldColor : Colors.grey),
                                onPressed: () {
                                  setState(() {
  ...
  Widget _buildEditableWrap({
    ...
    List<Widget> chips =
        items.where((item) => item['isHidden'] != true).map<Widget>((item) {
  ...
  Widget _buildInnerCircleContent() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: _innerCircleSections
          .where((s) => s['isHidden'] != true)
          .map((sectionMap) {
        final cat = sectionMap['title'] as String;
        final categoryItems =
            _innerCircle.where((e) => e['category'] == cat).toList();
            ...
            Wrap(
              ...
              children: [
                ...categoryItems
                    ...
                    .map((item) {
                  final chipColor = item['color'] != null
                      ? Color(item['color'])
                ...
                if ((cat == 'Partner' && categoryItems.isNotEmpty) ||
                  ...
                  ActionChip(
                    ...
                    backgroundColor: Colors.grey.shade200,
                    onPressed: () =>
                        _showAddInnerCircleDialog(defaultCategory: cat),
                    side: BorderSide.none,
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(8)),
                  )
  ...
  Widget _buildFloatingCard(
      ...
      VoidCallback? onMagicWand,
      VoidCallback? onCustomizeElements}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: goldAccent
            ? Border.all(color: _goldColor.withValues(alpha: 0.5), width: 1.5)
        ...
        children: [
          Row(
            children: [
              Icon(icon,
                  size: 18,
                  color: goldAccent ? _goldColor : Colors.grey.shade500),
              const SizedBox(width: 8),
              Text(title.toUpperCase(),
                  style: TextStyle(
                      color: goldAccent ? _goldColor : Colors.grey.shade700,
                      fontWeight: FontWeight.bold,
                      fontSize: 11,
                      letterSpacing: 1.0)),
              const Spacer(),
                ...
                InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(12)),
                    child: Row(children: [
                      Icon(Icons.auto_awesome, size: 12, color: _goldColor),
              ...
              InkWell(
                onTap: () =>
                    setState(() => _cardExpanded[cardKey] = !isExpanded),
                child: Padding(
                  padding: const EdgeInsets.all(4.0),
                  child: Icon(
  ...
  void _showBrainstormModal({
    ...
    try {
      localChatSession = GenerativeModel(
        ...
        apiKey: ApiKeyManager.geminiKey,
        systemInstruction: Content.system(
            "You are helping the user define their '$categoryTitle'. Their Prime Purpose is: '${_purposeController.text}'. $aiContext Keep responses brief, conversational, and direct."),
      ).startChat();

      localChatHistory.add(ChatMessage(
          text:
              "Hi! Let's brainstorm your $categoryTitle. Tell me what's on your mind, or ask me for suggestions.",
          isUser: false));
    } catch (e) {
      localChatHistory
          .add(ChatMessage(text: "AI is currently offline.", isUser: false));
    }
      ...
      builder: (ctx) => Dialog(
        ...
        child: StatefulBuilder(builder: (ctx, setModalState) {
          Future<void> sendLocalMessage() async {
            ...
            localChatCtrl.clear();
            WidgetsBinding.instance.addPostFrameCallback((_) => localScrollCtrl
                .jumpTo(localScrollCtrl.position.maxScrollExtent));

            try {
              final response =
                  await localChatSession.sendMessage(Content.text(msg));
              if (response.text != null) {
                setModalState(() {
                  localChatHistory
                      .add(ChatMessage(text: response.text!, isUser: false));
                  isLocalTyping = false;
                });
                      ...
                      Expanded(
                        child: Container(
                          decoration: BoxDecoration(
                              ...
                              border: Border.all(color: Colors.grey.shade200)),
                          child: TextField(
                            controller: localChatCtrl,
                            style: const TextStyle(color: Colors.black),
                            decoration: InputDecoration(
                                hintText: "Chat to brainstorm...",
                                hintStyle: TextStyle(
                  ...
                  Flexible(
                    child: ReorderableListView.builder(
                      ...
                      onReorder: (oldIndex, newIndex) {
                        setState(() {
                          if (newIndex > oldIndex) newIndex--;
                          final item = _cardOrder.removeAt(
                              _cardOrder.indexOf(filteredCardOrder[oldIndex]));
                          final newActualIndex = newIndex ==
                                  filteredCardOrder.length - 1
                              ? _cardOrder.length
              ...
              children: [
                ...
                Expanded(
                  child: _messages.isEmpty
                      ? Center(
                          child: Text("Ask me anything about your purpose...",
                ...
                if (_isLoading)
                  const Padding(
                      ...
                      child:
                          CircularProgressIndicator(color: Color(0xFFBB8E13))),
                Padding(
                  padding: const EdgeInsets.all(16),
                  child: Row(
                    children: [
                        ...
                        child: Container(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                                ...
                                border: InputBorder.none),
                            onSubmitted: (_) =>
                                _sendGlobalMessage(setDialogState),
                          ),
                        ),
                      ),
                      const SizedBox(width: 10),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: GlobalAppBar(
        ...
        onFilterTap: _showCardVisibilitySettings,
        onSortTap: () {},
      ),
